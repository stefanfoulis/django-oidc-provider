...
def migrate_response_type(apps, schema_editor):
    ...
    for client in Client.objects.all():
        client.response_types = [
            str(value)
            for value
            in client.old_response_types.values_list('value', flat=True)
        ]
        client.save()
...
def migrate_response_type_back_to_model(apps, schema_editor):
    ...
    for client in Client.objects.all():
        client.old_response_types.set(list(ResponseType.objects.filter(value__in=client.response_types)))
